...
/* TODO:
 *	- eventually separate momentum and position, only really need position in release mode
 *	- momentum measurement does not work properly without the CenterSite flag since q is not changed.
 */
...
public:
	...
	{
		vector<string> labels{"S","qx","qy","qz","N","SiteSet","Lattice","Model","MaxDim","Geometry","Connected","CenterSite"};	
	...
	{
		vector<string> labels{"S","i","j","rix","riy","riz","rjx","rjy","rjz","N","SiteSet","Lattice","Model","MaxDim","Geometry","Connected","CenterSite"};
		if(args->getString("Lattice") == "Triangular") { labels.push_back("Nx"); labels.push_back("Ny"); }
		return labels;
	}
