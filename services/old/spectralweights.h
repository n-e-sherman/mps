...
public:
    ...
    write(int n)
    {
        ...
        ofstream f;
        f.open(resDir + to_string(n));
        for(auto x : range(frequencies[n].size())) f << frequenciesBare[n][x] << "," << frequencies[n][x] << "," << weights[n][x] << "," << residues[n][x] << endl;
        f.close(); 
    }
